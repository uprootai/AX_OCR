...
async def call_edocr2_ocr(
    ...
    try:
        ...
        logger.info(f"Calling eDOCr2 v2 API for {filename} (content-type: {content_type})")
        logger.info(
        ...
        async with httpx.AsyncClient(timeout=600.0) as client:  # 10 minutes for large upscaled images
            ...
            logger.info(f"eDOCr2 v2 API status: {response.status_code}")
            if response.status_code == 200:
                ...
                logger.info(
                    f"eDOCr2 v2 results: {dimensions_count} dims, "
                ...
                raise HTTPException(
                    ...
                    detail=f"eDOCr2 v2 failed: {response.text}"
                )
