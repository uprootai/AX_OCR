...
| 서비스 | URL | 포트 | 설명 |
|--------|-----|------|------|
| Gateway API | `http://localhost:8000` | 8000 | 통합 오케스트레이션 |
| YOLOv11 API | `http://localhost:5005` | 5005 | 공학 도면 객체 검출 (GPU) |
| eDOCr2 v1 API | `http://localhost:5001` | 5001 | 한글 OCR (Fast, GPU) |
| eDOCr2 v2 API | `http://localhost:5002` | 5002 | 한글 OCR (Advanced, GPU) |
| PaddleOCR API | `http://localhost:5006` | 5006 | 다국어 OCR (GPU) |
| EDGNet API | `http://localhost:5012` | 5012 | 도면 세그멘테이션 (GPU) |
| Skin Model API | `http://localhost:5003` | 5003 | 공차 예측 및 제조 가능성 분석 |
| VL API | `http://localhost:5004` | 5004 | 비전-언어 멀티모달 분석 |

**참고**:
- EDGNet API는 Docker 내부에서 5002 포트로 실행되지만, 외부 접속은 5012 포트를 사용합니다.
- GPU 가속 API: YOLO, eDOCr2 v1/v2, PaddleOCR, EDGNet
