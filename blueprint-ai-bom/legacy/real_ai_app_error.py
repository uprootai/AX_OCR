...
def create_background_image_with_boxes_cached(image_array, detections_data, verification_status_hash):
    ...
    for idx, det_info in enumerate(detections_data):
        ...
        # 박스 오른쪽에 배치 (x2 + 10부터 시작)
        label_x = x2 + 10
        label_y = y1 + 30  # 박스 상단에서 약간 아래
...
# OCR 기능 제거됨
    ...
    def create_final_verified_image(self, detections, prefix):
        """최종 검증된 결과를 시각화 (승인/거부/수정/수작업 포함) - 캐싱 최적화"""
        ...
        # 검출 데이터와 검증 상태를 해시로 변환
        cache_data = {
            'detections': [
                {
                    'bbox': det.get('bbox', det.get('box', [])),
                    'original_index': det.get('original_index'),
                    'model': det.get('model', det.get('model_id', ''))
                }
                for det in detections
            ],
            'modified_classes': dict(st.session_state.get('modified_classes', {})),
            'image_path': st.session_state.current_image.get('path', '')
        }
        cache_hash = hashlib.md5(json.dumps(cache_data, sort_keys=True).encode()).hexdigest()

        # 캐시 확인
        cache_key = f'final_image_cache_{prefix}'
        if cache_key in st.session_state:
            cached_hash, cached_image = st.session_state[cache_key]
        ...
        for i, detection in enumerate(detections):
            ...
            if original_idx is not None:
                ...
                display_num = original_idx + 1  # 1부터 시작하는 번호
            else:
                status_key = f"{prefix}_{i}"
                display_num = i + 1  # 1부터 시작하는 번호
        ...
        # 캐시 저장
        st.session_state[cache_key] = (cache_hash, image)
            ...
            if not all_detections:
                ...
                with col1:
                    if st.button("🔘 모두 승인", key="approve_all_unified"):
                        ...
                        # 배치 작업 플래그 설정 (성능 최적화용)
                        st.session_state['batch_operation'] = True
                ...
                # 배치 작업인지 확인 (성능 최적화)
                is_batch_operation = st.session_state.get('batch_operation', False)

                # Ground Truth를 한번만 로드 (성능 개선) - 배치 작업 시 스킵
                ...
                if not is_batch_operation:
                    ...
                    # 배치 작업 시 간단하게 표시
                    self.render_detection_list(unique_detections, "unified", ground_truth=None)
                    # 플래그 해제
                    st.session_state['batch_operation'] = False
                ...
                # 최종 이미지 생성 (승인/수정/수작업만 표시) - 캐싱으로 최적화
    ...
    def render_detection_list(self, detections, prefix, show_voting=False, ground_truth=None):
        ...
        _start_render_list = time.time() if not is_batch_operation else 0
        _iou_time_total = 0
        ...
        with col_opt1:
            ...
            if show_gt_key not in st.session_state:
                # 배치 작업 시 기본값 False (성능 최적화)
                st.session_state[show_gt_key] = False if is_batch_operation else False
        ...
        with col_opt2:
            ...
            if show_ref_key not in st.session_state:
                # 배치 작업 시 기본값 False (성능 최적화)
                st.session_state[show_ref_key] = False if is_batch_operation else False
        ...
        if True:  # 항상 활성화
            ...
            try:
                ...
                # 캔버스와 심볼 테이블을 나란히 표시
                col_canvas, col_symbol_table = st.columns([3, 1])

                # 캔버스 생성 - 도면 이미지와 함께 표시 (왼쪽 컬럼)
