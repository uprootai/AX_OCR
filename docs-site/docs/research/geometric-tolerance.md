...
- **소속**:
  ...
  - Universite Paris-Saclay, ENS Paris-Saclay, LURPA (프랑스)
...
- **수식**:
  - In-plane shrinkage: Xs = Sp x X
  - Correlation: Mcr = exp(-d^2_ij / l^2_cr)
- **파라미터 조정**:
...
#### 1. 열 전도 방정식 (순차 결합 해석)
```
Q(x,t) - div q(x,t) = rho Cp (dT/dt)
```
- q = -k grad T (Fourier's law)
- Goldak's 3D Gaussian ellipsoidal distribution 사용

#### 2. 열 손실
- 대류: qconv = h(Ts - Ta)
- 복사: qrad = epsilon sigma (T^4_s - T^4_a)

#### 3. 기계적 응답 (선형 탄성 방정식)
```
div sigma = 0
sigma = C epsilon_e (Hooke's law)
epsilon = epsilon_e + epsilon_p + epsilon_t (총 변형률)
```
- 열 변형률: epsilon_t = alpha (T(x,y,z,t) - Tref)
...
- **층간 회전 각도**: 225deg
- **베이스 플레이트**: 100 x 100 x 20mm (중앙 고정)
- **계산 시간**:
  - Pin-ABTA: 5시간 25분
  - Hole-ABTA: 1시간 53분
