...
- **선택**: n=4 (계산 효율성, 안정성)
- **이유**: 대부분의 벡터가 직선이므로 더 많은 포인트가 추가 정보 제공 안 함
...
- **해결 방안**: Edge features 추가 (연결 각도, 위치 이동, 곡률 변화)

## 응용 분야

### 1. 자동화된 도면 해석
- 부품 형상 식별, 치수 추출, 제조 요구사항 파악

### 2. 견적 시스템
- 온라인 제조 플랫폼, 자동 견적 생성, 제조 가능성 평가

### 3. 도면 데이터베이스
- Content-based 검색, 유사 도면 찾기, 부품 인덱싱

### 4. CAD 통합
- 래스터에서 벡터 변환, 파라메트릭 모델 생성, 설계 정보 추출

## 한계점 및 향후 연구

### 현재 한계
1. **기호 인식 부재** - GD&T 기호, 표면 거칠기 기호 등
2. **Edge Features 부족** - 연결 타입 정보 없음
3. **고립 컴포넌트 처리** - 맥락 정보 활용 불가
4. **사용자 검증 필요** - 실제 사용자 테스트 미실시

### 향후 연구 방향
1. **기호 감지 알고리즘** - 휴리스틱 + 데이터 기반 방법
2. **향상된 그래프 표현** - Edge Features 추가, k-NN Edges
3. **고급 GNN 모델** - GAT, Graph Transformers, GINE Convolution
4. **사용자 인터페이스** - 도면 업로드, 자동 예측, 오류 수정
5. **확장 응용** - 제조 방법 분류, 치수 추정, 유사도 검색

## 결론

이 연구는 래스터 공학 도면 분석을 위한 새로운 프레임워크를 제시:

1. **혁신적 접근**: 이미지에서 그래프 변환, 희소 픽셀 문제 해결, 맥락 정보 효과적 임베딩
2. **우수한 성능**: 2-class 98.48%, 3-class 90.82%, Baseline 대비 5-11% 향상
3. **실용적 가치**: 제조 현장 적용 가능, 자동화된 견적 시스템
4. **확장 가능성**: 다른 도면 분석 task, 제조 방법 분류, 치수 추정

## AX 시스템 적용
- **사용 API**: EDGNet API (Port 5012)
- **용도**: 엣지 세그멘테이션, 공학 도면 컴포넌트 분류
- **기술**: GraphSAGE 기반 Graph Convolutional Network

## 참고 자료
- arXiv: https://arxiv.org/abs/2212.00290
