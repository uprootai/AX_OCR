...
> PDF 견적서의 페이지 레이아웃, 브랜딩 설정, 다중 페이지 처리 방식을 설명합니다.

PDF 보고서는 검증된 BOM 데이터를 기반으로 제조 견적서를 생성합니다. 보고서에는 회사 브랜딩, 구조화된 단가 테이블, 선택적 도면 이미지가 포함됩니다.
...
- [내보내기 형식](./export-formats.md) -- Excel, JSON 등 다른 내보내기 형식
- [BOM 및 견적](./index.mdx) -- 전체 BOM 파이프라인 개요
- [견적 엔진](./pricing-engine.mdx) -- 원가 계산 로직
