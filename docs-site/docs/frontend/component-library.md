...
|----------|-----------|-------|-------------|
| **UI Base** | `components/ui/` | 8 | 재사용 가능한 기본 UI 요소 |
| **BlueprintFlow** | `components/blueprintflow/` | 17 | 워크플로우 빌더 전용 (노드, 팔레트, 패널) |
| **Debug** | `components/debug/` | 10 | 개발용 디버그 도구 |
| **Upload** | `components/upload/` | 5 | 파일 업로드 (드래그앤드롭, 미리보기) |
| **Charts** | `components/charts/` | 4 | Recharts 기반 데이터 시각화 |
| **Monitoring** | `components/monitoring/` | 4 | API 상태, GPU, 리소스 모니터링 |
| **Guides** | `components/guides/` | 4 | 사용자 가이드 및 온보딩 |
| **Layout** | `components/layout/` | 3 | Header, Sidebar, Layout 프레임 |
| **Dashboard** | `components/dashboard/` | 3 | API 추가, 컨테이너 관리 다이얼로그 |
| **Visualization** | `components/visualization/` | 2 | 결과 시각화 오버레이 |
| **Common** | `components/common/` | 2 | CropRegionPreview, GenericOverlayViewer |
| **기타** | `pid/`, `bom/`, `results/` 등 | 7 | 도메인별 특화 컴포넌트 |

## BlueprintFlow Components
...
| `ApiNodes` | `blueprintflow/nodes/` | API 연동 노드 (Detection, OCR, Analysis) |
...
|-----------|-------------|
| `APIStatusMonitor` | 21개 API 서비스 실시간 상태 대시보드 (healthy/unhealthy/loading) |
...
| `Badge` | `color`, `label` | 상태 표시 뱃지 (healthy, error, pending) |
| `Card` | `title`, `children` | 컨텐츠 카드 컨테이너 |
...
- **Color Palette**: Tailwind 기본 팔레트 + 커스텀 확장
- **Dark Mode**: `class` 전략 (`dark:` prefix, `uiStore.theme`으로 토글)
- **Spacing**: 4px 그리드 시스템 (Tailwind 기본)
