...
```mermaid
flowchart LR
    NEW["New Predictions"] --> SCORE["Uncertainty\nScoring"]
    ...
    MED --> REVIEW
    REVIEW --> LOG["Verification Log\n(JSONL)"]
    LOG --> RETRAIN["Model Retrain"]
    LOW --> AUTO["Auto-Approve"]
```

## Priority Scoring Algorithm

`ActiveLearningService._determine_priority()` 는 각 항목의 confidence 와 관계(relation) 상태를 평가하여 4단계 우선순위를 결정합니다.
...
| Priority | Condition | Action | 예상 검토 시간/항목 |
|----------|-----------|--------|---------------------|
...
1. **CRITICAL** 항목을 confidence 오름차순으로 정렬 (가장 불확실한 것부터)
2. **HIGH** 항목을 confidence 오름차순으로 정렬
3. **MEDIUM** 항목을 confidence 오름차순으로 정렬
...
| Action | 의미 | 재학습 활용 |
|--------|------|------------|
| `approved` | 원본 정확 | positive sample |
| `rejected` | 오검출 | negative sample |
| `modified` | 수정됨 | corrected ground truth |
...
## Integration with Agent Verification

Active Learning은 Agent 검증 시스템의 L2 (Agent Review) 레벨과 연동됩니다:

1. 낮은 confidence 검출 → Active Learning 큐 등록
2. Agent (Claude Sonnet)가 우선 검토
3. Agent 불확실 → L3 Human Review로 에스컬레이션
