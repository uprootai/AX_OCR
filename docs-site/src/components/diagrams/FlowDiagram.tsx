    ...
    <div style={{
      ...
      <div style={{ position: 'relative', width: totalW, height: totalH, margin: '0 auto' }}>
        ...
        {nodes.map(node => {
          const p = pos[node.id];
          if (!p) return null;
          const color = node.color || 'var(--ifm-color-emphasis-700)';
          const rawBg = node.bg || 'var(--ifm-color-emphasis-100)';
          const bg = node.bg
            ? `color-mix(in srgb, ${node.bg} 55%, var(--ifm-background-color))`
            : rawBg;
            ...
            return (
              <div key={node.id} style={{
                ...
                <div style={{ position: 'relative', zIndex: 1, textAlign: 'center', lineHeight: 1.2 }}>
                  ...
                  {node.sub && <div style={{ fontSize: '0.65rem', color: 'var(--ifm-color-emphasis-600)' }}>{node.sub}</div>}
                </div>
              </div>
            );
          }
          ...
          return (
            <div key={node.id} style={{
              ...
              {node.sub && (
                <div style={{ fontSize: '0.65rem', color: 'var(--ifm-color-emphasis-600)', marginTop: '2px', textAlign: 'center' }}>
                  {node.sub}
                </div>
              )}
            </div>
        ...
        <svg style={{ position: 'absolute', top: 0, left: 0, width: totalW, height: totalH, pointerEvents: 'none' }}>
          ...
          {arrows.map((a, i) => (
            <g key={i}>
              <path d={a.d} stroke="var(--ifm-color-emphasis-400)" strokeWidth="1.5"
              ...
              {a.label && (
                <>
                  <rect x={a.lx - a.label.length * 3.5 - 4} y={a.ly - 9}
                    ...
                    fill="var(--ifm-background-color)" opacity={0.92} />
                  <text x={a.lx} y={a.ly + 1} textAnchor="middle" dominantBaseline="middle"
                    fontSize="11" fontWeight="600" fill="var(--ifm-color-emphasis-700)"
