...
> 사용자 피드백(승인/거부/수정)을 수집하고, 검증 완료 세션을 YOLO 재학습용 데이터셋으로 변환하는 파이프라인.
...
| 메서드 | 엔드포인트 | 설명 |
|--------|-----------|------|
| POST | `Gateway /qa/feedback` | 피드백 파이프라인 |

## 관련 문서

- [품질 보증](/docs/quality-assurance) — QA 체계 개요
- [능동 학습](/docs/quality-assurance/active-learning) — 피드백 파이프라인과 연동되는 능동 학습
