...
| 노드 | 포트 | 설명 | 주요 파라미터 |
|------|------|------|--------------|
| **YOLO** | 5005 | YOLOv11 심볼 검출 (Engineering 14개 / P&ID Symbol 32개 / BOM 27개 모델 지원) | `model_type`, `confidence`, `iou`, `imgsz`, `use_sahi`, `slice_height`, `slice_width`, `overlap_ratio`, `visualize`, `task` |
