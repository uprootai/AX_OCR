...
> API 서비스 스캐폴딩, 실행기 등록, 프론트엔드 노드 정의까지 5단계에 걸쳐 BlueprintFlow에 커스텀 API 노드를 추가하는 절차를 설명한다.
...
<FlowDiagram client:load
  ...
  title="커스텀 API 통합 단계"
  nodes={[
    { id: 'api', label: '1. API 서비스', sub: 'FastAPI 컨테이너', color: '#1565c0', bg: 'color-mix(in srgb, #e3f2fd 55%, var(--ifm-background-color))', shape: 'rounded' },
    { id: 'spec', label: '2. API 스펙', sub: 'YAML', color: '#37474f', bg: 'color-mix(in srgb, #eceff1 55%, var(--ifm-background-color))', shape: 'rounded' },
    { id: 'exec', label: '3. 실행기', sub: 'Gateway', color: '#e65100', bg: 'color-mix(in srgb, #fff3e0 55%, var(--ifm-background-color))', shape: 'rounded' },
    { id: 'nodedef', label: '4. 노드 정의', sub: 'TypeScript', color: '#1b5e20', bg: 'color-mix(in srgb, #e8f5e9 55%, var(--ifm-background-color))', shape: 'rounded' },
    { id: 'reg', label: '5. API 레지스트리', sub: 'TypeScript', color: '#1b5e20', bg: 'color-mix(in srgb, #e8f5e9 55%, var(--ifm-background-color))', shape: 'rounded' },
  ]}
  edges={[
    { from: 'api', to: 'spec', label: '백엔드' },
    { from: 'spec', to: 'exec' },
    { from: 'exec', to: 'nodedef', label: '프론트엔드' },
    { from: 'nodedef', to: 'reg' },
  ]}
  layers={[['api'], ['spec'], ['exec'], ['nodedef'], ['reg']]}
/>
...
| 메서드 | 엔드포인트 | 설명 |
|--------|-----------|------|
| POST | `Gateway /blueprintflow/execute` | 커스텀 실행 |

## 관련 문서

- [BlueprintFlow](/docs/blueprintflow/) -- BlueprintFlow 개요
- [노드 카탈로그](/docs/blueprintflow/node-catalog) -- 기존 노드 타입 전체 레퍼런스
- [DAG 엔진](/docs/blueprintflow/dag-engine) -- 실행기 레지스트리 구조
