...
tags: [프론트엔드, React, BOM, UI]
---

import FlowDiagram from '../../../components/diagrams/FlowDiagram';
import SequenceDiagram from '../../../components/diagrams/SequenceDiagram';

# BOM 프론트엔드 (BOM Frontend)

> 포트 3000에서 구동되는 BOM 전용 React 앱의 페이지 구조, 세션 생명주기, API 클라이언트 레이어를 설명합니다.
...
<FlowDiagram client:load
  direction="TD"
  title="BOM 프론트엔드 아키텍처"
  nodes={[
    { id: 'app', label: 'React 앱', sub: 'React Router', color: '#1565c0', bg: '#e3f2fd', shape: 'rounded' },
    { id: 'store', label: 'sessionStore', sub: 'Zustand', color: '#311b92', bg: '#ede7f6', shape: 'rounded' },
    { id: 'apilib', label: 'API 클라이언트 레이어', sub: 'Axios', color: '#00838f', bg: '#e0f7fa', shape: 'rounded' },
    { id: 'api', label: 'FastAPI 서버', sub: 'BOM 백엔드 :5020', color: '#e65100', bg: '#fff3e0', shape: 'rounded' },
    { id: 'sstore', label: '세션 저장소', color: '#e65100', bg: '#fff3e0', shape: 'rounded' },
    { id: 'gwapi', label: '파이프라인 API', sub: 'Gateway :8000', color: '#37474f', bg: '#eceff1', shape: 'rounded' },
  ]}
  edges={[
    { from: 'app', to: 'store' },
    { from: 'store', to: 'apilib' },
    { from: 'apilib', to: 'api', label: 'api 인스턴스' },
    { from: 'apilib', to: 'gwapi', label: 'gatewayApi 인스턴스' },
    { from: 'api', to: 'sstore' },
  ]}
  layers={[['app'], ['store'], ['apilib'], ['api', 'gwapi'], ['sstore']]}
/>
...
<SequenceDiagram client:load
  participants={[
    { id: 'user', label: '사용자', color: '#37474f' },
    { id: 'fe', label: 'BOM 프론트엔드', color: '#1565c0' },
    { id: 'store', label: 'sessionStore', color: '#311b92' },
    { id: 'be', label: 'BOM 백엔드 :5020', color: '#e65100' },
  ]}
  messages={[
    { from: 'user', to: 'fe', label: '이미지 업로드', type: 'sync' },
    { from: 'fe', to: 'store', label: 'uploadImage(file)', type: 'sync' },
    { from: 'store', to: 'be', label: 'POST /api/upload', type: 'sync' },
    { from: 'be', to: 'store', label: 'sessionId', type: 'reply' },
    { from: 'user', to: 'fe', label: '검출 실행', type: 'sync' },
    { from: 'fe', to: 'store', label: 'runDetection(config)', type: 'sync' },
    { from: 'store', to: 'be', label: 'POST /api/sessions/{id}/detect', type: 'sync' },
    { from: 'be', to: 'store', label: 'Detection[]', type: 'reply' },
    { from: 'user', to: 'fe', label: '검증 (승인/거부)', type: 'sync' },
    { from: 'fe', to: 'store', label: 'verifyDetection(id, status)', type: 'sync' },
    { from: 'store', to: 'be', label: 'PATCH /api/detections/{id}', type: 'sync' },
    { from: 'be', to: 'store', label: '업데이트된 검출 결과', type: 'reply' },
    { from: 'user', to: 'fe', label: 'BOM 내보내기', type: 'sync' },
    { from: 'fe', to: 'be', label: 'POST /api/export', type: 'sync' },
    { from: 'be', to: 'fe', label: 'Excel/PDF/JSON 다운로드', type: 'reply' },
  ]}
/>
...
- [BOM 및 견적](../bom-generation/index.mdx) -- BOM 백엔드 파이프라인
- [내보내기 형식](../bom-generation/export-formats.md) -- Excel/PDF/JSON 내보내기 사양
