...
# 설계 검사기 (Design Checker)

> 규칙 기반 P&ID 설계 검증 API(포트 5019) — BWMS 전용 50개 이상의 규칙으로 장비, 배관, 계기, 안전 사항을 자동 검증.
...
<FlowDiagram client:load
  ...
  title="규칙 엔진 아키텍처"
  nodes={[
    { id: 'input', label: 'P&ID 데이터', sub: '심볼 + 연결 + 텍스트', color: '#37474f', bg: '#eceff1', shape: 'rounded' },
    { id: 'profile', label: '규칙 프로필?', color: '#1565c0', bg: '#e3f2fd', shape: 'diamond' },
    { id: 'default', label: '기본 규칙', sub: '일반 P&ID', color: '#1565c0', bg: '#e3f2fd', shape: 'rounded' },
    { id: 'bwms', label: 'BWMS 규칙', sub: '50개 이상 전용', color: '#e65100', bg: '#fff3e0', shape: 'rounded' },
    { id: 'chemical', label: '화학 규칙', sub: '공정 안전', color: '#311b92', bg: '#ede7f6', shape: 'rounded' },
    { id: 'engine', label: '규칙 평가 엔진', color: '#1565c0', bg: '#e3f2fd', shape: 'rounded' },
    { id: 'pass', label: '통과(PASS)', sub: '규칙 충족', color: '#2e7d32', bg: '#e8f5e9', shape: 'rounded' },
    { id: 'warning', label: '경고(WARNING)', sub: '잠재적 문제', color: '#e65100', bg: '#fff3e0', shape: 'rounded' },
    { id: 'violation', label: '위반(VIOLATION)', sub: '규칙 위반', color: '#c62828', bg: '#ffebee', shape: 'rounded' },
    { id: 'report', label: '설계 검사 보고서', color: '#37474f', bg: '#eceff1', shape: 'stadium' },
  ]}
  edges={[
    { from: 'input', to: 'profile' },
    { from: 'profile', to: 'default', label: 'default' },
    { from: 'profile', to: 'bwms', label: 'bwms' },
    { from: 'profile', to: 'chemical', label: 'chemical' },
    { from: 'default', to: 'engine' },
    { from: 'bwms', to: 'engine' },
    { from: 'chemical', to: 'engine' },
    { from: 'engine', to: 'pass' },
    { from: 'engine', to: 'warning' },
    { from: 'engine', to: 'violation' },
    { from: 'pass', to: 'report' },
    { from: 'warning', to: 'report' },
    { from: 'violation', to: 'report' },
  ]}
  layers={[['input'], ['profile'], ['default', 'bwms', 'chemical'], ['engine'], ['pass', 'warning', 'violation'], ['report']]}
/>
...
| 메서드 | 엔드포인트 | 설명 |
|--------|-----------|------|
| POST | `Design Checker /api/v1/check` | 설계 검증 |

## 관련 문서

- [P&ID 분석 개요](/docs/pid-analysis) — P&ID 분석 파이프라인 전체 구조
- [연결성 분석](/docs/pid-analysis/connectivity) — 설계 검사의 입력이 되는 연결 그래프
- [심볼 검출](/docs/pid-analysis/symbol-detection) — 장비 존재 여부 확인의 기반
- [라인 검출](/docs/pid-analysis/line-detection) — 영역 검출 및 배관 규칙 검증의 기반
