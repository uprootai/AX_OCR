...
# P&ID 분석 (P&ID Analysis)

> YOLO v11 심볼 검출, OpenCV 라인 검출, 그래프 기반 연결성 분석, 규칙 기반 설계 검증으로 구성된 P&ID 도면 분석 파이프라인.
...
<PipelineDiagram client:load type="pid" />
...
전체 클래스 목록 및 모델 설정은 [심볼 검출](./symbol-detection.mdx)을 참조하세요.
...
자세한 API 문서는 [라인 검출](./line-detection.mdx)을 참조하세요.
...
그래프 알고리즘의 세부 사항은 [연결성 분석](./connectivity.mdx)을 참조하세요.
...
규칙 엔진 및 BWMS 전용 규칙에 대한 자세한 내용은 [설계 검사기](./design-checker.mdx)를 참조하세요.
...
- **Techcross API**: 장비 태그 추출(`/api/v1/techcross/equipment`), 밸브 태그 추출(`/api/v1/techcross/valves`), 표제란 파싱, Equipment List 매칭
...
| 메서드 | 엔드포인트 | 설명 |
|--------|-----------|------|
| POST | `PID Analyzer /api/v1/analyze` | P&ID 분석 |

## 관련 문서

- [심볼 검출](/docs/pid-analysis/symbol-detection) — YOLO v11 기반 60개 클래스 심볼 검출
- [라인 검출](/docs/pid-analysis/line-detection) — OpenCV 기반 라인 스타일 및 색상 분류
- [연결성 분석](/docs/pid-analysis/connectivity) — 그래프 기반 장비 연결 분석
- [설계 검사기](/docs/pid-analysis/design-checker) — BWMS 전용 50개 이상 규칙 검증
- [분석 파이프라인](/docs/analysis-pipeline) — 전체 도면 분석 파이프라인 개요
