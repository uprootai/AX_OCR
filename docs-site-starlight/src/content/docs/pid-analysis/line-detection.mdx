...
# 라인 검출 (Line Detection)

> OpenCV 기반 라인 검출기 API(포트 5016) — 6가지 라인 스타일 분류, 색상 분류, 영역 검출, 교차점 분석.
...
교차점은 [연결성 분석](./connectivity.mdx) 단계로 전달되어, 라인이 실제로 연결되어 있는지(티 분기) 또는 단순히 교차하는 것인지(브리지/크로스오버)를 판별하는 데 사용됩니다.
...
<FlowDiagram client:load
  ...
  title="파이프라인 통합"
  nodes={[
    { id: 'ld', label: '라인 검출기', sub: '포트 5016', color: '#2e7d32', bg: '#e8f5e9', shape: 'rounded' },
    { id: 'pa', label: 'PID 분석기', sub: '포트 5018', color: '#00838f', bg: '#e0f7fa', shape: 'rounded' },
    { id: 'ca', label: '연결성 분석기', color: '#e65100', bg: '#fff3e0', shape: 'rounded' },
    { id: 'dc', label: '설계 검사기', sub: '포트 5019', color: '#c62828', bg: '#fce4ec', shape: 'rounded' },
  ]}
  edges={[
    { from: 'ld', to: 'pa', label: 'lines[]' },
    { from: 'ld', to: 'ca', label: 'intersections[]' },
    { from: 'ld', to: 'dc', label: 'regions[]' },
  ]}
  layers={[['ld'], ['pa', 'ca', 'dc']]}
/>
...
| 메서드 | 엔드포인트 | 설명 |
|--------|-----------|------|
| POST | `PID Analyzer /api/v1/lines` | 배관 검출 |

## 관련 문서

- [P&ID 분석 개요](/docs/pid-analysis) — P&ID 분석 파이프라인 전체 구조
- [심볼 검출](/docs/pid-analysis/symbol-detection) — YOLO v11 기반 심볼 검출
- [연결성 분석](/docs/pid-analysis/connectivity) — 라인과 심볼을 결합한 연결 그래프
- [설계 검사기](/docs/pid-analysis/design-checker) — 영역 검출 기반 설계 검증
