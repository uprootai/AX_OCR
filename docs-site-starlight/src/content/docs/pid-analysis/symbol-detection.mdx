...
# 심볼 검출 (Symbol Detection)

> YOLO v11 기반 P&ID 심볼 검출 — 60개 클래스, SAHI 슬라이싱, NMS 병합을 통한 소형 심볼 검출.
...
<FlowDiagram client:load
  ...
  title="검출 파이프라인"
  nodes={[
    { id: 'image', label: 'P&ID 이미지', sub: '5000x3500', color: '#1565c0', bg: '#e3f2fd', shape: 'rounded' },
    { id: 'sahi', label: 'SAHI 슬라이싱', sub: '512x512, 25% 중첩', color: '#e65100', bg: '#fff3e0', shape: 'rounded' },
    { id: 'yolo', label: 'YOLO v11 추론', sub: '슬라이스별', color: '#2e7d32', bg: '#e8f5e9', shape: 'rounded' },
    { id: 'nms', label: 'NMS 병합', sub: 'IOU = 0.45', color: '#4a148c', bg: '#f3e5f5', shape: 'rounded' },
    { id: 'margin', label: '마진 페널티', sub: '표제란 = 0.5x', color: '#c62828', bg: '#fce4ec', shape: 'rounded' },
    { id: 'output', label: '심볼 검출 결과', sub: '60개 클래스', color: '#e65100', bg: '#fff3e0', shape: 'stadium' },
  ]}
  edges={[
    { from: 'image', to: 'sahi' },
    { from: 'sahi', to: 'yolo' },
  ...
  layers={[['image'], ['sahi'], ['yolo'], ['nms'], ['margin'], ['output']]}
/>
...
| 메서드 | 엔드포인트 | 설명 |
|--------|-----------|------|
| POST | `PID Analyzer /api/v1/symbols` | 심볼 검출 |

## 관련 문서

- [P&ID 분석 개요](/docs/pid-analysis) — P&ID 분석 파이프라인 전체 구조
- [라인 검출](/docs/pid-analysis/line-detection) — 배관 및 계장 라인 검출
- [연결성 분석](/docs/pid-analysis/connectivity) — 심볼 간 연결 그래프 구축
- [설계 검사기](/docs/pid-analysis/design-checker) — 규칙 기반 설계 검증
