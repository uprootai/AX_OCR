...
- [심볼 검출](/pid-analysis/symbol-detection) — 연결 그래프의 노드가 되는 심볼 검출
- [라인 검출](/pid-analysis/line-detection) — 연결 분석의 입력이 되는 라인 검출
- [설계 검사기](/pid-analysis/design-checker) — 연결 그래프 기반 설계 규칙 검증
- [BOM 생성](/bom-generation) — 연결성 데이터를 활용한 BOM 생성
