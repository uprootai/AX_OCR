...
> 심볼 검출과 라인 검출 결과를 결합하여 장비 연결 그래프를 구축하고, 흐름 경로를 추적하며, 고립 심볼을 식별하는 연결성 분석기.
...
<FlowDiagram client:load
  ...
  title="연결 그래프"
  nodes={[
    { id: 'p1', label: '펌프', sub: 'P-101', color: '#1565c0', bg: '#e3f2fd', shape: 'rounded' },
    { id: 'v1', label: '게이트 밸브', sub: 'V-101', color: '#2e7d32', bg: '#e8f5e9', shape: 'rounded' },
    { id: 'hx', label: '열교환기', sub: 'HX-101', color: '#e65100', bg: '#fff3e0', shape: 'rounded' },
    { id: 'v2', label: '제어 밸브', sub: 'CV-101', color: '#2e7d32', bg: '#e8f5e9', shape: 'rounded' },
    { id: 't1', label: '탱크', sub: 'T-101', color: '#1565c0', bg: '#e3f2fd', shape: 'rounded' },
    { id: 'pt', label: '압력 전송기', sub: 'PT-101', color: '#4a148c', bg: '#f3e5f5', shape: 'rounded' },
    { id: 'ft', label: '유량 전송기', sub: 'FT-101', color: '#4a148c', bg: '#f3e5f5', shape: 'rounded' },
  ]}
  edges={[
    { from: 'p1', to: 'v1', label: '실선' },
    { from: 'v1', to: 'hx', label: '실선' },
    { from: 'hx', to: 'v2', label: '실선' },
    { from: 'v2', to: 't1', label: '실선' },
    { from: 'pt', to: 'v1', label: '계기 신호', dashed: true },
    { from: 'ft', to: 'v2', label: '계기 신호', dashed: true },
  ]}
  layers={[['p1'], ['v1'], ['hx'], ['v2'], ['t1']]}
/>
...
| 메서드 | 엔드포인트 | 설명 |
|--------|-----------|------|
| POST | `PID Analyzer /api/v1/connectivity` | 연결성 |

## 관련 문서

- [P&ID 분석 개요](/docs/pid-analysis) — P&ID 분석 파이프라인 전체 구조
- [심볼 검출](/docs/pid-analysis/symbol-detection) — 연결 그래프의 노드가 되는 심볼 검출
- [라인 검출](/docs/pid-analysis/line-detection) — 연결 분석의 입력이 되는 라인 검출
- [설계 검사기](/docs/pid-analysis/design-checker) — 연결 그래프 기반 설계 규칙 검증
- [BOM 생성](/docs/bom-generation) — 연결성 데이터를 활용한 BOM 생성
