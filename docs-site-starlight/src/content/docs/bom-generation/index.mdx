...
# BOM 및 견적 (BOM & Quotation)

> 도면 업로드부터 검출, 검증, BOM 생성, 원가 산출까지 엔드투엔드 파이프라인을 설명합니다.
...
<PipelineDiagram client:load type="bom" />
...
- **L1 자동 승인**: 높은 신뢰도 검출(&ge; 0.9)은 자동으로 승인됩니다
...
[견적 엔진](./pricing-engine.mdx)이 제조 원가를 계산합니다:
...
- **YOLO 백엔드**: YOLO v11 (고속, ~40ms) — 도면 유형별 전용 모델 (engineering, panasia, pid_symbol)
...
| 메서드 | 엔드포인트 | 설명 |
|--------|-----------|------|
| POST | `Blueprint AI BOM /api/v1/detect` | BOM 검출 |

## 관련 문서

- [검출 클래스](./detection-classes.md) -- 73개 검출 클래스 전체 목록
- [견적 엔진](./pricing-engine.mdx) -- 원가 계산 로직 및 수식
- [내보내기 형식](./export-formats.md) -- Excel, JSON, PDF 내보내기 사양
