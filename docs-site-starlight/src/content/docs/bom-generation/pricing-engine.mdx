...
> 재료비, 가공비, 열처리비 등 제조 원가 계산 수식과 표준 부품 카탈로그 단가 조회 방식을 설명합니다.
...
<SequenceDiagram client:load
  title="원가 계산 흐름"
  participants={[
    { id: 'bom', label: 'BOM 데이터', color: '#1565c0' },
    { id: 'engine', label: '원가 계산기', color: '#e65100' },
    { id: 'catalog', label: '표준 부품 카탈로그', color: '#1b5e20' },
    { id: 'config', label: '단가 설정', color: '#311b92' },
  ]}
  messages={[
    { from: 'bom', to: 'engine', label: '항목 (설명, 재질, 크기, 수량)', type: 'sync' },
    { from: 'engine', to: 'catalog', label: '표준 부품 여부?', type: 'sync' },
    { from: 'catalog', to: 'engine', label: '카탈로그 단가', type: 'reply' },
    { from: 'engine', to: 'bom', label: '단가 x 수량', type: 'reply' },
    { from: 'engine', to: 'config', label: '재료 단가, 가공 요율 조회', type: 'sync' },
    { from: 'engine', to: 'engine', label: '치수 파싱 (OD/ID/L)', type: 'self' },
    { from: 'engine', to: 'engine', label: '여유량 적용', type: 'self' },
    { from: 'engine', to: 'engine', label: '중량 계산', type: 'self' },
    { from: 'engine', to: 'engine', label: '재료비 + 가공비 + 처리비 계산', type: 'self' },
    { from: 'engine', to: 'engine', label: '셋업비 + 검사비 + 운송비', type: 'self' },
    { from: 'engine', to: 'bom', label: '전체 원가 내역', type: 'reply' },
  ]}
  loops={[
    { label: '표준 부품 (볼트, 너트, 와셔, 핀, 플러그)', start: 2, end: 3 },
    { label: '주문 가공품', start: 4, end: 10 },
  ]}
/>
...
| 메서드 | 엔드포인트 | 설명 |
|--------|-----------|------|
| POST | `Blueprint AI BOM /api/v1/estimate` | 견적 |

## 관련 문서

- [BOM 및 견적](./index.mdx) -- 전체 BOM 파이프라인 개요
- [내보내기 형식](./export-formats.md) -- 원가 내역이 포함된 내보내기 형식
- [검출 클래스](./detection-classes.md) -- 견적 대상 검출 클래스 목록
