...
> 프로젝트 생성부터 견적 내보내기까지 8단계 API 호출을 자동화하는 CLI 도구 — BOM 견적, P&ID 분석, 일반 제조 도면 타입 지원.
...
<FlowDiagram client:load
  ...
  title="8단계 온보딩 파이프라인"
  nodes={[
    { id: 'p1', label: '1. 프로젝트', sub: '생성', color: '#1565c0', bg: '#e3f2fd', shape: 'rounded' },
    { id: 'p2', label: '2. BOM', sub: '임포트', color: '#1565c0', bg: '#e3f2fd', shape: 'rounded' },
    { id: 'p3', label: '3. 도면', sub: '매칭', color: '#1565c0', bg: '#e3f2fd', shape: 'rounded' },
    { id: 'p4', label: '4. 세션', sub: '생성', color: '#1b5e20', bg: '#e8f5e9', shape: 'rounded' },
    { id: 'p5', label: '5. 배치', sub: '시작', color: '#e65100', bg: '#fff3e0', shape: 'rounded' },
    { id: 'p6', label: '6. 상태', sub: '폴링', color: '#e65100', bg: '#fff3e0', shape: 'rounded' },
    { id: 'p7', label: '7. Agent', sub: '검증', color: '#c62828', bg: '#ffebee', shape: 'rounded' },
    { id: 'p8', label: '8. 견적', sub: '내보내기', color: '#311b92', bg: '#ede7f6', shape: 'rounded' },
  ]}
  edges={[
    { from: 'p1', to: 'p2' },
    { from: 'p2', to: 'p3' },
  ...
  layers={[['p1'], ['p2'], ['p3'], ['p4'], ['p5'], ['p6'], ['p7'], ['p8']]}
/>
...
| 메서드 | 엔드포인트 | 설명 |
|--------|-----------|------|
| GET | `Gateway /health` | 시스템 상태 확인 |

## 관련 문서

- [배치 처리 및 납품](/docs/batch-delivery) — 배치 처리 시스템 개요
- [배치 처리](/docs/batch-delivery/batch-processing) — 온보딩 5단계에서 호출되는 배치 분석
- [프로젝트 관리](/docs/batch-delivery/project-management) — 온보딩 1단계에서 생성되는 프로젝트 구조
- [내보내기 패키지](/docs/batch-delivery/export-package) — 온보딩 8단계의 견적 내보내기
