...
# 내보내기 패키지 (Export Package)

> JSON/Excel/PDF/HTML 납품 패키지 생성 및 Docker 이미지를 포함한 독립 실행 내보내기(Self-contained Export) 기능.
...
<FlowDiagram client:load
  direction="TD"
  title="8단계 Import 프로세스"
  nodes={[
    { id: 'a', label: '1. Docker 이미지 로드', sub: 'tar.gz → docker load', color: '#1565c0', bg: '#e3f2fd', shape: 'rounded' },
    { id: 'b', label: '2. Docker 네트워크 생성', color: '#1565c0', bg: '#e3f2fd', shape: 'rounded' },
    { id: 'c', label: '3. docker-compose up', color: '#1565c0', bg: '#e3f2fd', shape: 'rounded' },
    { id: 'd', label: '4. Nginx 설정 업데이트', sub: '프론트엔드 API 프록시', color: '#e65100', bg: '#fff3e0', shape: 'rounded' },
    { id: 'e', label: '5. 백엔드 헬스체크 대기', sub: '최대 30초', color: '#e65100', bg: '#fff3e0', shape: 'rounded' },
    { id: 'f', label: '6. 프로젝트 + GT 라벨 복원', color: '#1b5e20', bg: '#e8f5e9', shape: 'rounded' },
    { id: 'g', label: '7. 참조 GT 라벨 주입', sub: '컨테이너 내부', color: '#1b5e20', bg: '#e8f5e9', shape: 'rounded' },
    { id: 'h', label: '8. 세션 데이터 Import', sub: 'session_import.json', color: '#2e7d32', bg: '#e8f5e9', shape: 'rounded' },
  ]}
  edges={[
    { from: 'a', to: 'b' },
    { from: 'b', to: 'c' },
...
| 메서드 | 엔드포인트 | 설명 |
|--------|-----------|------|
| GET | `Gateway /projects/:id/export` | 패키지 내보내기 |

## 관련 문서

- [배치 처리 및 납품](/docs/batch-delivery) — 배치 처리 시스템 개요
- [프로젝트 관리](/docs/batch-delivery/project-management) — 프로젝트 계층 관리
- [배치 처리](/docs/batch-delivery/batch-processing) — 일괄 분석 후 내보내기 연동
- [고객 온보딩](/docs/batch-delivery/onboarding-guide) — 내보내기를 포함한 전체 파이프라인
